...
#'   \item{event}{Name of even in meet, just 100m here}
#'   \item{sex}{Sex of competitors}
...
#'
#' @examples
#'  # Declare factors, remove some missing values, split by sex
#' data(full100results)
#' library(tidyverse)
#' library(ggplot2)
...
#'   meet = factor(meet),
#'   event = factor(event),
#'   sex = factor(sex)) %>% drop_na(mark1, reactiontime) #Drop 54 and 1 extra missing on reaction time for missingness - note potential bias for removing these
#' f100results <- full100results %>% filter(sex == "Women")
#' m100results <- full100results %>% filter(sex == "Men")
